...
import {
  fillPageForm,
  readPageForm,
} from '../../internal/httpserver/web/assets/scripts/admin/page-settings.js';
import {
...
test('Telegram 模板以服务端响应为唯一默认来源', () => {
  ...
  assert.deepEqual(normalizeTelegramTemplates({
    'zh-CN': 'legacy zh',
    'en-US': 'legacy en',
  }), {
    'zh-CN': 'legacy zh',
...
test('新订阅使用中文服务端模板并复制 service_ids', () => {
  const templates = normalizeTelegramTemplates({ zh: 'zh template', en: 'en template' });
  const serviceIDs = ['s1'];
...
test('Telegram 测试在列表和编辑器中保留成功或失败反馈', async () => {
  ...
  const success = await sendTelegramTest({
    ...
    save: async options => { savedOptions = options; },
  ...
  const failure = await sendTelegramTest({
    ...
    results: [rowResult],
    save: async () => {},
    api: async () => { throw new Error('Telegram API returned: chat not found'); },
    toast: message => toasts.push(message),
  });
