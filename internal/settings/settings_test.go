...
func TestLoadDefaultsOnMissingFile(t *testing.T) {
	c, err := settings.Load(filepath.Join(t.TempDir(), "nope.yaml"))
	if err != nil {
...
func TestLoadOrCreatePublishesValidPrivateDefault(t *testing.T) {
	path := filepath.Join(t.TempDir(), "nested", "config.yaml")
	config, created, err := settings.LoadOrCreate(path)
	if err != nil {
		t.Fatal(err)
	}
	if !created {
	...
	loaded, created, err := settings.LoadOrCreate(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveRoundTrip(t *testing.T) {
	...
	if got.Telegram.Subscriptions[0].Template != notification.DefaultTemplate {
		t.Error("空通知模板应填充默认卡片")
	}
	if got.Telegram.Subscriptions[0].Language != notification.DefaultLanguage {
...
func TestValidateTelegramSubscriptions(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			cfg := settings.Config{Services: []model.Service{service}, Telegram: test.telegram}
			cfg.Normalize()
...
func TestExampleConfigLoads(t *testing.T) {
	config, err := settings.Load(filepath.Join("..", "..", "config.example.yaml"))
	if err != nil {
	...
	if strings.TrimSpace(config.Telegram.Subscriptions[0].Template) != strings.TrimSpace(notification.DefaultTemplate) {
		t.Error("示例配置中的模板必须与内置默认卡片一致")
	}
	if config.Telegram.Subscriptions[0].Language != notification.DefaultLanguage {
...
func TestSaveUsesPrivatePermissions(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Windows 不提供 Unix 文件权限语义")
	}
	path := filepath.Join(t.TempDir(), "config.yaml")
	cfg := &settings.Config{}
