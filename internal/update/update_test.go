...
func TestCloseCancelsBackgroundUpdate(t *testing.T) {
	...
	service := New(Options{
		BuildInfo:     BuildInfo{Version: "v1.0.0"},
		DeploymentTag: "latest",
		UpdateURL:     server.URL + "/v1/update?async=true",
		UpdateToken:   "secret",
		TriggerDelay:  time.Millisecond,
		UpdateWindow:  time.Minute,
	})
	if err := service.Start("v1.1.0"); err != nil {
		t.Fatal(err)
	}
	select {
	case <-started:
	...
	if err := service.Close(ctx); err != nil {
		t.Fatalf("关闭更新器失败: %v", err)
	}
	service.mu.Lock()
	updating := service.updating
	service.mu.Unlock()
	if updating {
		t.Fatal("关闭后 updating 应清零")
	}
	if err := service.Start("v1.1.0"); err == nil || !strings.Contains(err.Error(), "shutting down") {
		t.Fatalf("关闭后不应接受新更新: %v", err)
	}
}
