...
func TestUpdateEndpoints(t *testing.T) {
	...
	defer st.Close()
	sch := scheduler.New(st, nil)
	cfg := &config.Config{AdminToken: testToken, Page: model.PageConfig{HistoryLen: 60}}
	updates := updater.New(updater.Options{
		BuildInfo:        updater.BuildInfo{Version: "v1.0.0", Commit: "abc123"},
	...
	srv, err := New(Options{
		Scheduler: sch, Updater: updates,
		ConfigPath: filepath.Join(t.TempDir(), "config.yaml"), AdminToken: testToken,
	}, cfg)
	if err != nil {
		t.Fatal(err)
	}
