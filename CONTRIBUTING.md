...
开发环境固定使用 Go `1.25.13` 和 Node.js `24.13.1`。Node.js 开发依赖用于 JavaScript lint、格式校验和 OpenAPI 契约校验，不参与运行镜像。
...
- JavaScript 使用 ESLint 和 Prettier；API 路由或数据结构变化时同步更新 `api/openapi.yaml`。
...
`make ci` 会检查格式、模块一致性、`go vet`、race-enabled tests、JavaScript lint/格式、OpenAPI 契约、前端回归测试和已知漏洞。仅修改后端测试时也应执行完整门禁，因为前端资源嵌入 Go 二进制，发布产物是一个整体。
